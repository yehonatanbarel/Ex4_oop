...
"""  TRY NOT TO PUT THEM IN THE GRAPH AT FIRST
...
while client.is_running() == 'true':
    ...
    for ag_node in lst_of_agent_agent.values():
        if ag_node.get_dest() == -1:
            next_node = (ag_node.get_src() - 1) % graph_algo.get_graph().v_size()

            client.choose_next_edge(
                '{"agent_id":'+str(ag_node.get_id())+', "next_node_id":'+str(next_node)+'}')
