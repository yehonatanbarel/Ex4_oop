...
for n in range(len(graph['Nodes'])):
    x, y, _ = graph['Nodes'][n]['pos'].split(',')
    graph['Nodes'][n]['pos'] = SimpleNamespace(x=float(x), y=float(y))
...
for i in range(len(graph['Nodes'])):
    x_lst.append(graph['Nodes'][i]['pos'].x)
for i in range(len(graph['Nodes'])):
    y_lst.append(graph['Nodes'][i]['pos'].y)
...
while client.is_running() == 'true':
    ...
    # draw nodes
    for n in range(len(graph['Nodes'])):
        x = my_scale(graph['Nodes'][n]['pos'].x, x=True)
        y = my_scale(graph['Nodes'][n]['pos'].y, y=True)
        ...
        # draw the node id
        id_srf = FONT.render(str(graph['Nodes'][n]['id']), True, Color(255, 255, 255))
