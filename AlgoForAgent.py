...
## ======================= HERE I PUT ALL THE LIST I AM USING IN THIS WORK ==================
...
def get_num_of_agent_to_put_on_screen():
...
make_lst_pokemon_pokemon()
make_lst_pokemon_node()
# make_lst_agent_agnet()

# def create_from_all_graph_algo():
graph_json = client.get_graph()
graph_check = json.loads(graph_json)#, object_hook=lambda json_dict: SimpleNamespace(**json_dict))
    ...
    for ag in lst_of_agent_agent.values():
        if ag.get_dest() == -1:  # if this agent dont have where to go
            ...
            for p in lst_of_pokemon_node.values():
                dist = GraphAlgo.shortest_path(agent_id, p.get_key())
